  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (message.toLowerCase().includes('invalid email')) {
        ...
        setError(t("common.phoneNotExist"));
      } else {
        setError(message || t("signIn.invalidCreds"));
      }
    } finally {
      setLoading(false);
    }
        ...
        <div className="w-full max-w-xl bg-white md:bg-gray-50/30 md:border md:border-gray-100 md:shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-[2rem] md:p-10 lg:p-12 relative z-10">
          ...
          <form className="w-full flex flex-col gap-5 md:gap-6" onSubmit={handleLogin}>
            ...
            <div className="w-full">
              <label className="block text-[13px] md:text-sm font-medium text-gray-800 mb-1.5 md:mb-2">{t("signIn.emailOrPhone")}</label>
              <div className={`relative flex items-center border rounded-2xl px-4 py-3.5 md:py-4 bg-white transition-colors shadow-sm ${error ? 'border-red-400' : 'border-gray-200 focus-within:border-[#449339]'}`}>
                <input
                  ...
                  name="identifier" placeholder={t("signIn.enterEmailOrPhone")}
            ...
            {/* Password */}
            <div className="w-full">
              <label className="block text-[13px] md:text-sm font-medium text-gray-800 mb-1.5 md:mb-2">{t("signIn.password")}</label>
              <div className={`relative flex items-center border rounded-2xl px-4 py-3.5 md:py-4 bg-white transition-colors shadow-sm ${error ? 'border-red-400' : 'border-gray-200 focus-within:border-[#449339]'}`}>
                <input
                  type={showPassword ? "text" : "password"}
                  value={password}
                  onChange={(e) => { setPassword(e.target.value); setError(''); }}
                  name="password" placeholder={t("signIn.enterPassword")}
                  className="w-full outline-none text-[14px] md:text-[15px] text-gray-900 placeholder:text-gray-400 bg-transparent"
                />
                <button
                  type="button"
            ...
            <div className="mt-4 w-full">
              ...
              >
                {loading ? t("signIn.signingIn") : t("common.login")}
              </button>
            </div>
            ...
            <div className="w-full mt-6 mb-2 flex items-center">
              ...
              <span className="px-4 text-[12px] md:text-sm text-gray-500 font-medium whitespace-nowrap">{t("common.orContinueWith")}</span>
              <div className="flex-grow h-px bg-gray-200"></div>
            </div>
            ...
            <div className="w-full grid grid-cols-2 gap-4 mb-4">
              <button type="button" className="flex items-center justify-center gap-2 py-3 border border-gray-200 rounded-full hover:bg-gray-50 hover:shadow-sm transition-all cursor-pointer bg-white">
                <svg className="w-5 h-5" viewBox="0 0 24 24">
                  ...
                  <path d="M5.84 14.09c-.26-.78-.4-1.61-.4-2.46s.14-1.68.4-2.46V6.6H2.18C1.43 8.08 1 9.98 1 12s.43 3.92 1.18 5.4l3.66-3.31z" fill="#FBBC05"/>
            ...
            <div className="text-center w-full mt-2">
              <p className="text-[13px] md:text-[14px] text-gray-800 font-medium">
                {t("common.dontHaveAccount")} <Link href="/auth/recycler/sign-up" className="font-bold text-[#1b5030] hover:text-[#449339] transition-colors hover:underline underline-offset-2">{t("common.signUp")}</Link>
              </p>
            </div>
